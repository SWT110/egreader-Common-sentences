...
const SOURCE_CSV = path.join(ROOT, 'en_top_sentences.csv');
const STATIC_TSV = path.join(ROOT, 'data', 'common_sentences_zh.tsv');
...
const WORD_ZH = Object.freeze({
  ...
  sir:'先生',maam:'女士',madam:'女士',man:'伙计',guys:'大家',friend:'朋友',friends:'朋友们',dad:'爸爸',daddy:'爸爸',mom:'妈妈',mommy:'妈妈',mother:'母亲',father:'父亲',brother:'兄弟',sister:'姐妹',son:'儿子',honey:'亲爱的',baby:'宝贝',doctor:'医生',captain:'队长',chief:'长官',boss:'老板',police:'警察',professor:'教授',president:'总统',
...
function loadRowsFromTsv() {
  if (!fs.existsSync(STATIC_TSV)) return [];
  const lines = fs.readFileSync(STATIC_TSV, 'utf8').replace(/^\uFEFF/, '').split(/\r?\n/).filter(Boolean);
  const header = lines.shift().split('\t');
  return lines.map((line) => {
    ...
    return {
      ...
      source: row.source || 'en_top_sentences.csv',
      sourceLine: Number(row.sourceLine) || null,
    };
  });
}

function loadCommonSentences() {
  if (!fs.existsSync(SOURCE_CSV)) return loadRowsFromTsv();
